...
describe("ChatModeSelector Component Unit Tests", () => {
  it("exports CHAT_MODE_OPTIONS with expected 3 modes", () => {
    expect(CHAT_MODE_OPTIONS.length).toBe(3);
  ...
  it("renders default 普通对话 mode when imageMode and agentEnabled are false", () => {
  ...
  it("selects 生图模式 and triggers callbacks accurately", () => {
    const onImageModeChange = mock((_val: boolean) => {});
    const onAgentEnabledChange = mock((_val: boolean) => {});
    ...
    const options = Array.from(container.querySelectorAll('[role="option"]'));
    const imageOption = options.find((opt) =>
      opt.textContent?.includes("生图模式"),
    );
    ...
    expect(onImageModeChange).toHaveBeenCalledWith(true);
