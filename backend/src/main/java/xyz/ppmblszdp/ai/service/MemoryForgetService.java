...
public class MemoryForgetService {
	 ...
	 */
	public double calculatePriorityScore(Double basePriority, Integer accessCount, String lastAccessedAt, String updatedAt) {
	...
	public List<String> compressMemories(String category, List<String> fineGrainedMemories) {
		...
		try {
			...
			userContent.append("分类: ").append(category != null ? category : "未分类").append("\n");
			userContent.append("细粒度记忆条目:\n");
			for (int i = 0; i < fineGrainedMemories.size(); i++) {
				userContent.append(i + 1).append(". ").append(fineGrainedMemories.get(i)).append("\n");
			}
			...
			if (response != null && !response.isBlank()) {
				...
				if (summaries != null && !summaries.isEmpty()) {
					log.info("记忆摘要压缩完成: 分类={}, 原始条数={}, 压缩后条数={}", category, fineGrainedMemories.size(), summaries.size());
					return summaries;
				}
			}
		} catch (Exception e) {
