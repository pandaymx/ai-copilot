...
public class IntentClassifier {
	...
	private static final Pattern MATH_PATTERN = Pattern.compile(
			"(?i)(\\\\frac|\\\\int|\\\\sum|\\\\matrix|\\\\sqrt|\\\\\\(|\\\\\\[|求解|求积分|求导|计算|方程|微分|" +
			"矩阵|概率|行列式|几何|算术|数学题|\\b[0-9]+[\\+\\-\\*\\/\\=][0-9]+\\b)"
	);

	private static final Pattern TRANSLATION_PATTERN = Pattern.compile(
			"(?i)(翻译[：:]?|translate|to\\s+english|to\\s+chinese|翻成中文|翻成英文|日文怎么说|英文怎么说|韩文怎么说|" +
			"把.*译成|中译英|英译中)"
	);

	private static final Pattern SEARCH_PATTERN = Pattern.compile(
			"(?i)(搜索|查找|搜一下|查一下|查查|最新新闻|实时资讯|天气预报|最新价格|最新消息|2026年|发生了什么|热搜)"
	);

	private static final Pattern WRITING_PATTERN = Pattern.compile(
			"(?i)(写一篇|作文|周报|新闻稿|演讲稿|邮件模板|润色|改写|公文|总结|提炼|观后感|邀请函|报告|文章)"
	);

	private static final Pattern ANALYSIS_PATTERN = Pattern.compile(
			"(?i)(分析|对比|评估|优缺点|架构设计|可行性|深度解析|归纳|SWOT|权衡|为什么|原因)"
	);
	...
	public IntentResult buildDefaultResult(IntentType intent) {
		return switch (intent) {
			case CODE -> new IntentResult(
					IntentType.CODE,
					false,
					true,
					"【智能路由 - 代码优化模式】请提供清晰、规范、高效的代码实现，包含关键代码注释，并解释主要逻辑与时间复杂度。"
			);
			case TRANSLATION -> new IntentResult(
					IntentType.TRANSLATION,
					false,
					false,
					"【智能路由 - 翻译模式】请进行准确、通顺、信达雅的翻译，保持原文语气与格式，无需进行多余问候。"
			);
			case WRITING -> new IntentResult(
					IntentType.WRITING,
					false,
					false,
					"【智能路由 - 创作模式】请根据要求输出结构清晰、文笔优美、段落分明的文本，注意排版与语气调整。"
			);
			case SEARCH -> new IntentResult(
					IntentType.SEARCH,
					true,
					true,
					"【智能路由 - 搜索分析模式】请优先利用搜索与外部工具检索最新资料，确保信息的时效性与准确性。"
			);
			case MATH -> new IntentResult(
					IntentType.MATH,
					false,
					true,
					"【智能路由 - 数学推导模式】请给出分步骤的推导与计算过程，准确使用 LaTeX 公式格式格式化数学符号。"
			);
			case ANALYSIS -> new IntentResult(
					IntentType.ANALYSIS,
					true,
					true,
					"【智能路由 - 深度分析模式】请从多维度进行系统化分析，对比优缺点与风险，给出客观、全面的结论。"
			);
			case MULTIMODAL -> new IntentResult(
					IntentType.MULTIMODAL,
					false,
					false,
					"【智能路由 - 多模态模式】请结合用户上传的媒体内容进行细致解析与说明。"
			);
			case IMAGE -> new IntentResult(
					IntentType.IMAGE,
					false,
					false,
					"【智能路由 - 绘图创作模式】提炼图像 Prompt 并调用图像生成组件。"
			);
			case CHAT -> new IntentResult(
					IntentType.CHAT,
					false,
					false,
					null
