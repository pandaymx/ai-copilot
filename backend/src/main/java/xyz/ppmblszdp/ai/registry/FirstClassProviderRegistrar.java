...
public class FirstClassProviderRegistrar {
    ...
    public Map<String, ProviderDescriptor> register() {
        ...
        registerOne(
                ...
                true,
                deepseek,
                properties.resolveFirstClass().get("deepseek"));
        registerOne(
                result,
                "openai",
                openaiApiKey,
                true,
                openai,
                properties.resolveFirstClass().get("openai"));
        registerOne(
                result,
                "google",
                googleApiKey,
                true,
                google,
                properties.resolveFirstClass().get("google"));
        registerOne(
                result,
                "ollama",
                null,
                false,
                ollama,
                properties.resolveFirstClass().get("ollama"));
        registerOne(
                result,
                "anthropic",
                anthropicApiKey,
                true,
                anthropic,
                properties.resolveFirstClass().get("anthropic"));
        return result;
    }
    ...
    private <T extends ChatModel> void registerOne(
        ...
        if (requiresApiKey && ApiKeyValidator.isPlaceholder(apiKey)) {
            xyz.ppmblszdp.ai.identity.AuthProperties authProps =
                    authPropertiesProvider != null ? authPropertiesProvider.getIfAvailable() : null;
        ...
        Map<String, ModelDescriptor> models =
                buildModelIndex(modelCfgs, properties.resolveContext().resolveDefaultMaxContextTokens());
        if (models.isEmpty()) {
            log.warn("一等公民供应商 '{}' 未声明任何可用模型，跳过注册", providerId);
            return;
        }
        String displayName =
                (cfg != null && cfg.displayName() != null && !cfg.displayName().isBlank())
                        ? cfg.displayName()
                        : providerId;
