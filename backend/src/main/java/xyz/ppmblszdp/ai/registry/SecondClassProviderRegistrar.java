...
public class SecondClassProviderRegistrar {
    ...
    private ProviderDescriptor registerOne(AiProviderProperties.SecondClassConfig cfg) {
        ...
        ChatModelFactory factory = factories.stream()
                ...
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("找不到支持协议 '%s' 的 ChatModelFactory".formatted(protocol)));
        ...
        for (ModelConfig mc : modelCfgs) {
            ...
            ModelDescriptor md =
                    ModelDescriptor.from(mc, properties.resolveContext().resolveDefaultMaxContextTokens());
            models.put(md.id(), md);
        }
        if (models.isEmpty()) {
