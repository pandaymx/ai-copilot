...
public class ImageModelRegistry {
    ...
    public ImageModelRegistry(ObjectProvider<ImageModel> imageModelProvider, AiProviderProperties properties) {
        ...
        if (this.defaultImageModel != null) {
            log.info(
                    "已成功装配全局默认 ImageModel: {}",
                    this.defaultImageModel.getClass().getSimpleName());
        } else {
     ...
     */
    public ImageOptions buildOptions(
            String providerId, String modelId, Integer width, Integer height, String quality, String style) {
        ...
        String pId = (providerId != null && !providerId.isBlank())
                ? providerId.toLowerCase().trim()
                : imgConfig.resolveDefaultProvider();
