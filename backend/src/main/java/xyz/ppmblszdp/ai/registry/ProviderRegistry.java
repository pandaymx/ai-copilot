...
public final class ProviderRegistry {
    ...
    public ResolvedModel resolve(String providerId, String modelId) {
        ...
        throw new ModelNotFoundException(
                providerId, modelId, new ArrayList<>(provider.models().keySet()));
    }
    ...
    public ResolvedModel resolveFallback(String currentProviderId, String fallbackProviderId, String fallbackModelId) {
        String primaryPid = (currentProviderId == null || currentProviderId.isBlank())
                ? defaultProviderId
                : currentProviderId.trim();

        if (fallbackProviderId != null
                && !fallbackProviderId.isBlank()
                && !fallbackProviderId.trim().equalsIgnoreCase(primaryPid)) {
            try {
        ...
        if (defaultProviderId != null
                && !defaultProviderId.equalsIgnoreCase(primaryPid)
                && providers.containsKey(defaultProviderId)) {
            try {
        ...
        public ProviderRegistry build() {
            ...
            if (defaultProviderId != null && providers.containsKey(defaultProviderId)) {
                ...
                if (defaultModelId == null
                        || defaultModelId.isBlank()
                        || (pd != null && pd.models() != null && !pd.models().containsKey(defaultModelId))) {
