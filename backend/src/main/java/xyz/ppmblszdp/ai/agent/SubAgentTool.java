...
public class SubAgentTool {
    ...
    private static final String CODE_SYSTEM_PROMPT =
            "你是一个专注于代码生成、重构与调试的 AI 助手。" + "请根据用户需求输出高质量、可运行的代码，并附上简短说明。" + "不要进行闲聊，直接给出代码与必要注释。";

    private static final String SUMMARY_SYSTEM_PROMPT =
            "你是一个专注于信息提炼、文档摘要与知识整合的 AI 助手。" + "请将用户提供的内容压缩为精炼、结构化的摘要，抓住关键要点。" + "不要发散，保持简洁。";
     ...
     */
    @Tool(description = "调度「数据分析型子代理」：适用于数据分析、统计、逻辑推理、计划拆解等任务。" + "请将需要分析的子任务完整描述在 task 中，子代理将独立执行并返回分析结论。")
    public String dispatchToAnalysisAgent(
            @ToolParam(description = "子任务描述：向子代理传递完整的分析需求，尽量提供充足背景信息") String task, ToolContext toolContext) {
     ...
     */
    @Tool(description = "调度「代码生成型子代理」：适用于代码生成、重构、解释、调试等编程任务。" + "请将编程需求完整描述在 task 中，子代理将独立输出高质量代码。")
    public String dispatchToCodeAgent(
            @ToolParam(description = "子任务描述：向子代理传递完整的编程需求，包含语言、功能目标与约束条件") String task, ToolContext toolContext) {
        SubAgentWorkerContext workerCtx = resolveWorkerCtx(toolContext);
     ...
     */
    @Tool(description = "调度「摘要整合型子代理」：适用于长文档摘要、知识提炼、信息整合等任务。" + "请将需要摘要整合的内容或任务描述完整放入 task，子代理将输出精炼的摘要。")
    public String dispatchToSummaryAgent(
            @ToolParam(description = "子任务描述：向子代理传递需要摘要或整合的内容与任务要求") String task, ToolContext toolContext) {
        SubAgentWorkerContext workerCtx = resolveWorkerCtx(toolContext);
    ...
    private SubAgentWorkerContext resolveWorkerCtx(ToolContext toolContext) {
        ...
        throw new IllegalStateException("ToolContext 中缺少 SubAgentWorkerContext（key=" + SubAgentWorkerContext.CTX_KEY
