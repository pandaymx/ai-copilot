    ...
    public static class AugmentedToolCallback implements ToolCallback {
                    ...
                    try {
                        ...
                        if (node instanceof ObjectNode objNode) {
                            if (objNode.has(INNER_THOUGHT_KEY)) {
                                ...
                                String thought =
                                        thoughtNode != null && !thoughtNode.isNull() ? thoughtNode.asText() : "";
                        ...
                        log.warn(
                                "[{}] Unstrict JSON parsing fallback for innerThought: {}, cause: {}",
        ...
        private ToolDefinition augmentSchema(ToolDefinition original) {
            ...
            try {
                ...
                log.warn(
                        "[{}] Failed to augment tool JSON schema with innerThought, returning original schema. Cause: {}",
                        original.name(),
                        e.getMessage());
            }
            return original;
