...
public class ToolSearchAdvisorConfig {
	...
	public ToolSearchAdvisorFactory toolSearchAdvisorFactory(
		...
		return new ToolSearchAdvisorFactory(properties, embeddingModelProvider, springAiAdvisorEnabled, springAiIndexType, springAiMinThreshold);
	}
	...
	public static class ToolSearchAdvisorFactory {
		...
		public ToolSearchAdvisorPropertiesConfig resolveConfig() {
			ToolSearchAdvisorPropertiesConfig appConfig = properties.resolveAgent().resolveToolSearchAdvisor();
		 ...
		 * @return 处理后的工具回调数组
		 */
		public ToolCallback[] processTools(ToolCallback[] allTools, int totalLocalCount, int totalMcpCount, String conversationId) {
			...
			log.info("ToolSearchAdvisor activated: indexed {} tools (local: {}, remote MCP: {}), session: {}, indexType: {}, threshold: {}",
					allTools.length, totalLocalCount, totalMcpCount, conversationId, effectiveIndexType, config.resolveMinToolsThreshold());
