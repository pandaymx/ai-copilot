...
public class ChatController {
    ...
    public Mono<ResponseEntity<byte[]>> tts(@RequestBody TtsRequest request, ServerWebExchange exchange) {
        ...
        return chatService
                .synthesizeSpeech(request.text(), request.voice(), userId)
                .map(audio -> ResponseEntity.ok()
                        .contentType(MediaType.parseMediaType("audio/mpeg"))
                        .body(audio))
                .onErrorResume(e ->
                        Mono.error(new ResponseStatusException(HttpStatus.BAD_GATEWAY, "语音合成失败：" + e.getMessage(), e)));
    }
    ...
    public Mono<String> transcribe(@RequestBody MediaDto audio, ServerWebExchange exchange) {
        ...
        String mime = (audio.mimeType() != null && !audio.mimeType().isBlank()) ? audio.mimeType() : "audio/webm";
        byte[] bytes;
        try {
        ...
        return chatService
                .transcribeAudio(bytes, mime, userId)
                .onErrorResume(e ->
                        Mono.error(new ResponseStatusException(HttpStatus.BAD_GATEWAY, "语音识别失败：" + e.getMessage(), e)));
    }
    ...
    public Mono<ChatResponseDto> chat(@RequestBody ChatRequest request, ServerWebExchange exchange) {
        ...
        return chatService.chat(request, userId).onErrorResume(AiException.class, ex -> Mono.error(ex));
    }
    ...
    public Flux<ServerSentEvent<ChatChunkDto>> stream(@RequestBody ChatRequest request, ServerWebExchange exchange) {
        ...
        Flux<ServerSentEvent<ChatChunkDto>> head = emitConversation
                ? Flux.just(ServerSentEvent.builder(ChatChunkDto.conversation(conversationId))
                        .build())
                : Flux.empty();
        return head.concatWith(chatService.streamChatChunks(request, userId).map(chunk -> ServerSentEvent.builder(chunk)
                        .build()))
                ...
                .onErrorResume(Exception.class, ex -> {
                    ...
                    return Flux.just(ServerSentEvent.builder(ChatChunkDto.error("UPSTREAM_ERROR", ex.getMessage()))
                            .build());
                });
    }
    ...
    public Mono<Map<String, Object>> feedback(@RequestBody ChatFeedbackRequest request, ServerWebExchange exchange) {
        String userId =
                UserIdentityFilter.resolveIdentity(exchange, request == null ? null : request.userId(), authProperties);
