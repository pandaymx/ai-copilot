...
class SafeGuardAdvisorTest {
    ...
    void testRequestPrivacyMask() {
        ...
        when(chain.nextCall(any())).thenAnswer(invocation -> {
            ...
            UserMessage userMsg =
                    (UserMessage) passedReq.prompt().getInstructions().get(0);
    ...
    void testAdviseStreamSanitization() {
        ...
        Flux<ChatClientResponse> streamSource =
                Flux.just(buildResponse("这里是"), buildResponse("包含违规"), buildResponse("词的输出文本。"));
