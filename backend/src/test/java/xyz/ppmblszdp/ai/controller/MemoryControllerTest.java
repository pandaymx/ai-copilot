...
class MemoryControllerTest {
    ...
    void listMemories_shouldReturnItemsForCurrentUser() {
        ...
        when(memoryService.listMemories(eq(USER), isNull(), eq("active"), anyInt(), anyInt()))
                .thenReturn(resp);
    ...
    void updateMemory_shouldReturn404_whenNotOwnerOrMissing() {
        when(memoryService.updateMemory(eq("m1"), eq(USER), any(), any(), any(), any()))
    ...
    void resolveConflicts_shouldReturnResolvedCount() {
        ...
        webClient
                .post()
                .uri("/api/memory/resolve-conflicts")
                .header("X-User-Id", USER)
                .exchange()
                .expectStatus()
                .isOk()
