...
class RagIngestionConflictTest {
    ...
    void ingest_shouldExecuteDeleteFirst_whenConflictPolicyIsOverwrite() {
        ...
        when(mockReaderFactory.read(SourceType.TEXT, rawText, "overwrite.txt"))
    ...
    void ingest_shouldForceAddAll_whenConflictPolicyIsForceAdd() {
        ...
        when(mockReaderFactory.read(SourceType.TEXT, rawText, "force.txt"))
                .thenReturn(List.of(rawDoc));

        var result = ingestionService.ingest(
                SourceType.TEXT, rawText, "force.txt", "user-002", ConflictPolicy.FORCE_ADD);
